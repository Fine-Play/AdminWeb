...
export default function AdminMatchesPage(){
  ...
  // 팀별 그룹핑(선수 탭) — teamName 필드가 있으면 그룹 정렬
  ...
  // ── 상세 Drawer UI ─────────────────────────────────────────────────────────
  const DetailDrawer = () => (!detail ? null : (
        ...
        {groupedPlayers.map((g, gi)=>(
          <Card key={gi}>
            ...
            <Table>
              <thead>
                <tr>
                  <th style={{width:90}}>userId</th>
                  <th>이름</th>
                  <th>포지션</th>
                  <th>매치포지션</th>
                  {["SPD","PAS","PAC","SHO","DRV","DEC","DRI","TAC","BLD","CRO","HED","FST","ACT","OFF","TEC","COP"].map(k=>(
                    <th key={k} style={{width:56}}>{k}</th>
                  ))}
                  <th style={{width:66}}>run</th>
                  <th style={{width:56}}>G</th>
                  <th style={{width:56}}>A</th>
                  <th style={{width:72}}>rating</th>
                </tr>
              </thead>
              <tbody>
                {g.rows.map((p, i)=>(
                  <tr key={`${gi}-${p.userId}-${i}`}>
                    <td>{p.userId}</td>
                    <td>{p.userName||"-"}</td>
                    <td>{p.position||"-"}</td>
                    <td>
                      <Select value={p.matchPosition||""}
                    ...
                    <td><Input type="number" min="0" value={p.runTime??""} onChange={e=>setPlayers(list=>list.map(x=>x.userId===p.userId?{...x, runTime:e.target.value}:x))}/></td>
                    <td><Input type="number" min="0" value={p.score??""} onChange={e=>setPlayers(list=>list.map(x=>x.userId===p.userId?{...x, score:e.target.value}:x))}/></td>
                    <td><Input type="number" min="0" value={p.assist??""} onChange={e=>setPlayers(list=>list.map(x=>x.userId===p.userId?{...x, assist:e.target.value}:x))}/></td>
                    <td><Input type="number" min="0" step="0.1" value={p.rating??""} onChange={e=>setPlayers(list=>list.map(x=>x.userId===p.userId?{...x, rating:e.target.value}:x))}/></td>
                  </tr>
                ))}
              </tbody>
            </Table>
